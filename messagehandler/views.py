...
def talktome_view(request):
    ...
    # Open the log file in append mode and write the current time to it
    with open('/var/log/messaging_system.log', 'a') as log_file:
        log_file.write(f"{current_time}\n")
    return JsonResponse({'status': 'Time logged'})  # Return a JSON response indicating the time has been logged

def logs_view(request):
    """
    This view handles the /logs endpoint. It reads the contents of the log file and returns them in a JSON response.
    """
    # Open the log file in read mode and read its contents
    with open('/var/log/messaging_system.log', 'r') as log_file:
        logs = log_file.read()
    return JsonResponse({'logs': logs})  # Return a JSON response with the logs